	...
	<view :class="disabled ? 'uni-list-item--disabled' : ''" :hover-class="disabled || showSwitch ? '' : 'uni-list-item--hover'" class="uni-list-item" @click="onClick">
		<view class="uni-list-item__container">
			...
			<view v-if="showBadge || showArrow || showSwitch" class="uni-list-item__extra">
				<text v-if="rightText" class="rightText">{{rightText}}</text>
				<uni-badge v-if="showBadge" :type="badgeType" :text="badgeText" />
				<switch v-if="showSwitch" :disabled="disabled" :checked="switchChecked" @change="onSwitchChange" />
				<text v-if="leftText" class="leftText">{{leftText}}</text>
				<uni-icons v-if="showArrow" :size="20" class="uni-icon-wrapper" color="#000" type="arrowright" />
			</view>
		</view>
	</view>
</template>
